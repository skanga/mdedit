(function(root) {
  ...
  root.MDEdit.installFormattingToolbar = function({document, format}) {
    ...
    const icons = {
      ...
      quote: '<path d="M10 7H4v6h5c0 3-2 4-4 4M20 7h-6v6h5c0 3-2 4-4 4"/>',
    ...
    const disclosure = (id, label, face, items, className = '') =>
      `<span class="format-disclosure ${className}"><button type="button" aria-label="${label}" title="${label}" aria-expanded="false" aria-controls="format-menu-${id}">${face}<span class="format-caret" aria-hidden="true">▾</span></button><div id="format-menu-${id}" class="format-menu" hidden>${items}</div></span>`;
    ...
    bar.innerHTML = disclosure('headings', 'Heading', 'H', Array.from({length:6}, (_, i) => command('heading-' + (i+1), 'Heading ' + (i+1), 'Heading ' + (i+1), mod + 'Alt+' + (i+1))).join('')) +
      ...
      disclosure('more', 'More formatting', icon('more'),
        ...
        command('rule', 'Horizontal rule') + command('equation', 'Equation'));
